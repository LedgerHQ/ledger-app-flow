...
There are two possibilities. You may build in a build container or not. Normally, you should use build container. For this you need to have environment variable `BOLOS_SDK=` (that is, empty). If you want to do non-containerized build you se this variable to the directory containing `nanos-secure-sdk` (but many make targets work only with containerized approach).
