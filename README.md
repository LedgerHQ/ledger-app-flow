...
- Open a terminal and clone `app-flow` with `git clone git@github.com:LedgerHQ/app-flow.git`.
...
- Install python 3
- Install the required SDK (e.g. under `/opt/`) and select the one to use:
  - `export BOLOS_SDK=/opt/nanos-secure-sdk`
- Install other dependencies running:
  - `make -C deps/ledger-zxlib/ deps`
...
```bash
make -C deps/ledger-zxlib/ generate_test_vectors
```
...
```bash
make -C deps/ledger-zxlib/ cpp_test
```
...
```bash
make -C deps/ledger-zxlib/ speculos_install
```

Then you can run JS tests. The tests compare against saved snapshots.

```bash
make -C deps/ledger-zxlib/ speculos_port_5001_test
make -C deps/ledger-zxlib/ speculos_port_5002_test
```

Or for NanoX:

```bash
TARGET_DEVICE=NANO_X make -C deps/ledger-zxlib/ speculos_port_5001_test
TARGET_DEVICE=NANO_X make -C deps/ledger-zxlib/ speculos_port_5002_test
```

Or for NanoSPlus:

```bash
TARGET_DEVICE=NANO_SP make -C deps/ledger-zxlib/ speculos_port_5001_test
TARGET_DEVICE=NANO_SP make -C deps/ledger-zxlib/ speculos_port_5002_test
```

If you want to change the snapshots you can set `TEST_PNG_RE_GEN_FOR` environment variable. If you want to change snapshots set `TEST_PNG_RE_GEN_FOR=test`, otherwise you can set a prefix and tests with that prefix will be re-generated. Dont forget to review generated snapshot.
