...
typedef enum { SLOT_OP_SET, SLOT_OP_UPDATE, SLOT_UP_DELETE } slotop_t;

account_slot_t tmp_slot;
uint8_t tmp_slotIdx;
slotop_t tmp_slotop;

slot_store_t NV_CONST N_slot_store_impl __attribute__((aligned(64)));
        ...
        case SLOT_OP_SET: {
            switch (displayIdx) {
                ...
                case 2: {
                    ...
                    path_options_to_string(bufferUI,
                                           sizeof(bufferUI),
                                           tmp_slot.path.data,
                                           HDPATH_LEN_DEFAULT,
                                           tmp_slot.options);
...
zxerr_t slot_parseSlot(uint8_t *buffer, uint16_t bufferLen) {
    ...
    STATIC_ASSERT(sizeof(apduData->account) == sizeof(tmp_slot.account),
    ...
    STATIC_ASSERT(sizeof(apduData->options) == sizeof(tmp_slot.options),
                  "Incompatible option types.");
    MEMCPY(&tmp_slot.options, &apduData->options, sizeof(apduData->options));

    const bool mainnet =
        tmp_slot.path.data[0] == HDPATH_0_DEFAULT && tmp_slot.path.data[1] == HDPATH_1_DEFAULT;
    const bool testnet =
        tmp_slot.path.data[0] == HDPATH_0_TESTNET && tmp_slot.path.data[1] == HDPATH_1_TESTNET;
...
zxerr_t slot_serializeSlot(const account_slot_t *slot, uint8_t *buffer, uint16_t *bufferLen) {
    ...
    STATIC_ASSERT(sizeof(apduData->account) == sizeof(slot->account),
                  "Incompatible account types.");
...
void loadAddressFromSlot(uint8_t hasHdPath) {
    ...
    if (err == zxerr_no_data) {
        ...
        STATIC_ASSERT(sizeof(slot.path.data) == sizeof(hdPath.data),
                      "Incompatible derivation path types");
        if (hasHdPath && (memcmp(slot.path.data, hdPath.data, sizeof(hdPath.data)) ||
                          ((slot.options & 0xFF00) !=
                           (cryptoOptions & 0xFF00)))) {  // curve portion of cryptoOptions
            show_address = SHOW_ADDRESS_HDPATHS_NOT_EQUAL;
        } else {
            // Case 3 Everything is OK
            STATIC_ASSERT(sizeof(address_to_display.data) == sizeof(slot.account.data),
            ...
            if (!hasHdPath) {
                STATIC_ASSERT(sizeof(hdPath.data) == sizeof(slot.path.data),
                              "Incompatible derivation path types");
