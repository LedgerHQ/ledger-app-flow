...
__Z_INLINE void handleGetPubkey(volatile uint32_t *flags, volatile uint32_t *tx, uint32_t rx) {
    ...
    STATIC_ASSERT(sizeof(G_io_apdu_buffer) > SECP256_PK_LEN + 2 * SECP256_PK_LEN + 1,
                  "IO Buffer too small");
    ...
    uint32_t len = array_to_hexstr((char *) (G_io_apdu_buffer + SECP256_PK_LEN),
                                   remainingLength,
                                   pubkey_to_display,
                                   sizeof(pubkey_to_display));
...
__Z_INLINE void handleGetSlot(__Z_UNUSED volatile uint32_t *flags,
                              volatile uint32_t *tx,
                              uint32_t rx) {
    if (rx != 6) {
        THROW(APDU_CODE_DATA_INVALID);
